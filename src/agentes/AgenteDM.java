...
public class AgenteDM extends Agent {
	...
	public class aplicarClasificacion extends OneShotBehaviour {
		...
		public void action() {
			ACLMessage msg = this.myAgent.blockingReceive();// accede al agente que tenga la accion y lo bloquea para
															// que solo quede esperando el mensaje
			...
			try {
				...
				Evaluation evalJ48 = new Evaluation(data);// Creamos un objeto para la validacion del modelo con
															// redBayesiana
				/* Aplicamos el clasificador J48 */
				evalJ48.crossValidateModel(j48, data, 10, new Random(1));// hacemos validacion cruzada con 10 campos, y
																			// el aleatorio
				...
				resultado.addReceiver(new AID("pantalla", AID.ISLOCALNAME));// AID= Agent identification, se le añade a
																			// quien se le envia
